...
def plot_timeseries(timeSeries):
    ...
    fig, axes = plt.subplots(nrows=4, ncols=2, dpi=180, figsize=(16, 10))
...
def plot_forecast(predictedArray, algo):
    ...
    axes.set_title("Forecast of Bitcoin by {}".format(algo), size=18)
    axes.set_xlabel("Dates", size=15)
    axes.set_ylabel("BTC/USD", size=15)

    plt.show()


def trading(predictedArray):
    ...
    portfolioHoldings = pd.DataFrame(
        data={
            ...
            "Bitcoins": np.array(cryptoHoldings).cumsum(),
        },
        index=predictedArray.index,
    )

    fig, ax = plt.subplots(nrows=2, ncols=1, figsize=(16, 8))
    fig.suptitle("Trading Strategy for Bitcoin", size=18)
    ax[0].plot(portfolioHoldings.Value)
    ax[0].set_title("Value of Portfolio", size=15)
    ax[0].set_ylabel("USD")

    ax[1].plot(portfolioHoldings.Bitcoins)
    ax[1].set_title("Bitcoin Holdings")
    ax[1].set_ylabel("Number of Bitcoins")
...
def printt(portfolioHoldings):
    ...
    print(
        "\t \t \t The value of your portfolio is {}".format(
            round(portfolioHoldings.Value[-1], 6)
        )
    )
    print(
        "\t \t \t You have achieved returns of {}%".format(
            round(
                (
                    (portfolioHoldings.Value[-1] - portfolioHoldings.Value[0])
                    / portfolioHoldings.Value[0]
                )
                * 100,
                2,
            )
